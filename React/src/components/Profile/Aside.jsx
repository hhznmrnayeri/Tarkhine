    ...
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 my-6 md:my-12">
        ...
        <div className={`top-20 bottom-0 ${showMenu?'left-0':'left-full'} z-10 fixed md:static bg-white w-full h-full md:h-80 px-5  md:col-span-3 md:flex flex-col md:border border-gray-400 md:px-2 md:py-4 md:rounded-lg transition-all duration-300`}>
            ...
            <ul className="flex flex-col pr-2 gap-1 text-xs md:text-sm mt-2">
                ...
                {/* state item */}
                <button onClick={openLogoutModal} className="text-error flex items-center gap-1 py-2.5 md:py-1.5">
                    {/* state icon */}
...
{showLogoutModal&&<Overlay onHide={closeLogoutModal}>
<div className="fixed overflow-hidden rounded-lg bg-white w-11/12 md:w-5/12 h-48 md:h-56 inset-0 m-auto z-30">
        ...
        {/* caption */}
        <p className="mt-3 md:mt-8 text-xs md:text-base text-center">آیا مایل به خروج از حساب کاربری خود هستید؟</p>
        {/* btn wrapper */}
        <div className="mt-8 flex-center gap-4 md:gap-5 px-10">
            {/* back btn */}
            <button onClick={closeLogoutModal} className="bg-primary rounded flex-center p-2 text-white text-xs md:px-4 md:text-base md:font-estedadMedium flex-1">بازگشت</button>
            {/* delete btn */}
            <NavLink to='/' className="bg-error-200 rounded flex-center p-2 text-error text-xs md:px-4 md:text-base md:font-estedadMedium flex-1">خروج</NavLink>
        </div>
    </div>
</Overlay>}
</>
