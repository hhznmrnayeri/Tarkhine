...
<div className="container">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 my-6 md:my-12">
                {/* state wrapper */}
                <div className="hidden top-20  bottom-0  fixed md:static bg-white w-full h-full md:h-80 px-5  md:col-span-3 md:flex flex-col md:border border-gray-400 md:px-2 md:py-4 md:rounded-lg transition-all duration-300">
                    {/* top wrapper */}
                    ...
                    <ul className="flex flex-col pr-2 gap-1 text-xs md:text-sm mt-2">
                        {/* state item */}
                        <button onClick={()=>setMenuItemActive('user')} className={`${menuItemActive==='user'?'profile__state--active':''} flex items-center gap-1 py-2.5 md:py-1.5`}>
                            {/* state icon */}
                        ...
                        {/* state item */}
                        <button onClick={()=>setMenuItemActive('wallet')} className={`${menuItemActive==='wallet'?'profile__state--active':''} flex items-center gap-1 py-2.5 md:py-1.5`}>
                            {/* state icon */}
                        ...
                        {/* state item */}
                        <button onClick={()=>setMenuItemActive('heart')} className={`${menuItemActive==='heart'?'profile__state--active':''} flex items-center gap-1 py-2.5 md:py-1.5`}>
                            {/* state icon */}
                        ...
                        {/* state item */}
                        <button onClick={()=>setMenuItemActive('location')} className={`${menuItemActive==='location'?'profile__state--active':''} flex items-center gap-1 py-2.5 md:py-1.5`}>
                            {/* state icon */}
...
{showMenu&&<Overlay onHide={closeAsideMenu} backgroundNone={true}>
<div className={`${showMenu?'left-0':'left-full'} top-20 bottom-0 fixed bg-white w-full h-full px-5 flex flex-col transition-all duration-300`}>
    {/* top wrapper */}
    <div className="flex items-center md:items-end gap-2 md:gap-5 pb-2 border-b border-gray-600">
        {/* img */}
        <img src="src/assets/images/profile/Ellipse.webp" alt="user" className="w-12 h-12 lg:w-20 lg:h-20"/>
