...
<div className="col-span-1 sm:col-span-6 xl:col-span-3 flex flex-col rounded md:rounded-lg border border-gray-400 overflow-hidden">
    {/* result img */}
    <img src={props.img} alt="result" className="result__img object-cover w-full"/>
    {/* result content */}
    <div className="bg-white p-4 flex-center flex-col gap-4">
        {/* result name */}
        <h2 className="result__name text-xs md:text-xl font-estedadSemiBold">{props.title}</h2>
        ...
        <div className="flex items-start justify-between w-full">
            ...
            <div className="flex flex-col items-start gap-1 text-2xs text-gray-500">
                ...
                <div className="flex items-center gap-1">
                    ...
                    {/* rate number */}
                    <span className="rate__number text-2xs md:text-sm text-gray-800 font-estedadMedium">{props.star}</span>
                    {/* points */}
                    <span>(<span className="points">{props.points}</span> امتیاز)</span>
                </div>
            </div>
            {/* left box */}
            <div className="flex flex-col items-end gap-1">
                ...
                {/* offer price */}
                <span className="text-2xs md:text-base"><span className="offer__price">{props.offer} </span>تومان</span>
            </div>
        </div>
        {/* btn result */}
        <button className="text-2xs md:text-base md:font-estedadMedium text-white px-4 py-2 w-full bg-primary flex-center rounded">افزودن به سبد خرید</button>
    </div>
</div>
)
}